...
echo "=== Validating standalone manifests in ${KUBERNETES_DIR}/flux-system ==="
find "${KUBERNETES_DIR}/flux-system" -maxdepth 1 -type f -name '*.yaml' -print0 | while IFS= read -r -d $'\0' file; do
...
find "${KUBERNETES_DIR}" -type f -name $kustomize_config -print0 | while IFS= read -r -d $'\0' file; do
